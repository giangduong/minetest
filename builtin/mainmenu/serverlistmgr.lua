...
local function get_favorites_path(folder)
	...
	local base = core.get_builtin_path() .. DIR_DELIM .. "serverlist" .. DIR_DELIM
	if folder then
		return base
	end
...
--------------------------------------------------------------------------------
function serverlistmgr.get_favorites(username,pass)
	...
	-- fav.name = "Build A City!"
	-- fav.description = "With Leanbot build Pythaverse in a city!"
	-- fav.address = "qa-mt.leanbot.space"
	-- fav.port = 30002

	-- serverlistmgr.favorites[1] = fav

	-- local fav1 = {}

	-- fav1.name = "Build A World!"
	-- fav1.description = "With Leanbot build Pythaverse in a world!"
	-- fav1.address = "qa-mt.leanbot.space"
	-- fav1.port = 30001

	-- serverlistmgr.favorites[2] = fav1

	

	local body = {}
	-- body.client_id = SSO.client_id
	-- body.client_secret = SSO.client_secret
	...
	local tokenObj = callFunction(SSO.url,"POST",header,false,body)
	local token = tokenObj.access_token

	--core.log("Token: "..token)

	local url =PYTHAVERSE.url.."/get?jsonRequest="..PYTHAVERSE.jsonRequest
	
	local world = callFunction(url,"GET",{"Authorization: Bearer "..token},true,{})

	
	
	for i,k in ipairs(world.data) do
		local fav = {}
		fav.name = k.title
		fav.description = k.desc
		fav.address = k.url
		fav.port = tonumber(k.port)
