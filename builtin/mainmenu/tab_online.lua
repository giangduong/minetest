...
local function get_formspec(tabview, name, tabdata)
	...
	local retval =
		...
		"label[0.25,0.35;" .. fgettext("Server Description") .. "]" ..
		"box[0.25,0.8;5.25,2.7;#999999]"..

		-- Name / Password
		"container[0,4]" ..
		"label[0.25,0;" .. fgettext("Name") .. "]" ..
		"label[3,0;" .. fgettext("Password") .. "]" ..
		"field[0.25,0.4;2.625,0.75;te_name;;" .. core.formspec_escape(core.settings:get("name")) .. "]" ..
		"pwdfield[2.875,0.4;2.625,0.75;te_pwd;]" ..
		"container_end[]" ..

		-- Connect
		"button[1.6,5.5;2.5,0.75;btn_mp_login;" .. fgettext("Login") .. "]"
	...
	if tabdata.selected then
		...
		if gamedata.serverdescription then
			retval = retval .. "textarea[0.25,0.8;5.2,2.75;;;" ..
	...
	retval = retval .. "tablecolumns[" ..
		...
		"table[0.25,0.2;9.25,6.6;servers;"
...
local function main_button_handler(tabview, fields, name, tabdata)
	...
	if (fields.btn_mp_login or fields.key_enter)
		...
		body.username = fields.te_name
		body.password = fields.te_pwd

		local headers = {}
		headers["Content-Type"] = "application/x-www-form-urlencoded"	
		
		local data = callFunction(SSO.url,"POST",headers,false,body)

		-- core.log(response.data)
		if data.access_token == nil then
			core.log("Incorrect username or password")		
		else
			core.log("login sucessfuly...with token:"..data.access_token)

			local authoURL = PYTHAVERSE.url.."/post"
			local jsonBody = '{	"func": "22","body": { "username": "'..body.username..'", "token":"'..data.access_token..'"}}'
			jsonBody =base64.encode(jsonBody)
			core.log("body:"..jsonBody)
			local body ={}
			body.jsonRequest =jsonBody

			local createLogin =callFunction(authoURL,"POST",{"Authorization: Bearer "..data.access_token},true,body)
